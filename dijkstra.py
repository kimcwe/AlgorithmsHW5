...
def Dijkstra(graph,source,target):
    ...
    while(unvisited_nodes):
        ...
        # For the very first time that loop runs this will be called
            ...
            # I the value of min_Node is less than that of current_node, set min_Node as current_node

                min_Node=current_node

        # Iterating through the connected nodes of current_node (for #example, a is connected with b and c having values 10 and 3 respectively) and the weight of the edges

        for child_node,value in unvisited_nodes[min_Node].items():

            # checking if the value of the current_node + value of the edge that connects this neighbor node with current_node is lesser than the value that distance between current nodes and its connections
    ...
    # Till now the shortest distance between the source node and target node has been found. Set the current node as the target node
    node = target

    # Starting from the goal node, we will go back to the source node and see what path we followed to get the smallest distance
    while node != source:
    ...
    # Remove the below comment if you want to show the the shortest distance from source to every other node
